            ...
            <div className="lg:col-span-2">
              {loading ? (
                ...
                <div className="bg-surface border border-border rounded-xl p-12 text-center text-text-muted">
                  No quotes found. Submit one via the{' '}
                  <Link href="/quote" className="text-primary">
                    quote form
                  </Link>
                  .
                </div>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            ...
                            {q.depositPaid && (
                              <span className="text-green-400 text-xs font-mono bg-green-900/30 border border-green-800 px-2 py-0.5 rounded-full">
                                Deposit Paid
                              </span>
                            )}
                          </div>
                          ...
                          <p className="text-text-muted text-xs truncate">
                            ...
                            {q.moveDate || 'Date TBD'} · {q.serviceType}
                          </p>
                        </div>
                        ...
                        <div className="text-right shrink-0">
                          {q.estimatedPrice && (
                            <p className="text-primary font-heading">
                              {formatCurrency(q.estimatedPrice)}
                            </p>
                          )}
            ...
            <div className="lg:col-span-1">
              {selected ? (
                <div className="bg-surface border border-border rounded-xl p-5 sticky top-24">
                  ...
                  <div className="space-y-2 mb-5 text-sm">
                    {[
                      ...
                      [
                        'From',
                        `${selected.originAddress ? selected.originAddress + ', ' : ''}${selected.originCity}`,
                      ],
                      [
                        'To',
                        `${selected.destinationAddress ? selected.destinationAddress + ', ' : ''}${selected.destinationCity}`,
                      ],
                      ...
                      <div key={label} className="flex justify-between gap-2">
                        ...
                        <span className="text-white text-right capitalize truncate">
                          {value}
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="mt-4 flex gap-2">
                    <a
                      ...
                      className="flex-1 text-center text-sm border border-border text-text-muted hover:border-primary hover:text-primary py-2 rounded transition-colors"
                    >
                      Email Client
                    </a>
                ...
                <div className="bg-surface border border-border rounded-xl p-8 text-center text-text-muted sticky top-24">
                  <p className="text-sm">
                    Select a quote to view full details and update its status.
                  </p>
                </div>
              )}
            </div>
